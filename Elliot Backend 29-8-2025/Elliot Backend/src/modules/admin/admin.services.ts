...
class adminServices {
    ...
    static async editUser(id: any, req_data: any, file: any) {
        try {
            ...
            const projection = { __v: 0 }
            const options = { lean: true }
            const check: any = await DAO.getData(Models.Users, { _id: req_data._id }, projection, options);
            if (!check.length) {
                throw await handleCustomError("NO_DATA_FOUND", language)
            }
    ...
    static async changePassword(user_id: string, old_password: string, new_password: string, language: string) {
        try {
            ...
            const users: any[] = await DAO.getData(Models.Admin, { _id: user_id }, projection, options);
            if (!users.length) {
                throw await handleCustomError("NO_DATA_FOUND", language);
            }
